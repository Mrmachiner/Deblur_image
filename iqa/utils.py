...
class Help:
	...
	def _get_sums(GT,P,win,mode='same'):
		mu1,mu2 = (Help().filter2(GT,win,mode),Help().filter2(P,win,mode))
	...
	def _get_sigmas(GT,P,win,mode='same',**kwargs):
		...
		return Help().filter2(GT * GT,win,mode)  - GT_sum_sq,\
				Help().filter2(P * P,win,mode)  - P_sum_sq, \
				Help().filter2(GT * P,win,mode) - GT_P_sum_mul
	@staticmethod
